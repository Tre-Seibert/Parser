Attribute VB_Name = "HELP01_MakeTicket_from_Rule_5y"
'############################################################################################################
'## HELP01_MakeTicket_from_Rule - VBA Code launched from Outlook Rule                                      5y
'############################################################################################################
...
Dim m_asSpamRules()         As String
Dim m_iRowsSpamRules        As Integer
...
Private Sub HELP_ProcessEmail(ByRef oMail As MailItem)
    ...
    Dim iSpamRow        As Integer      '-- stores row of spam un array if found
...
'    iFlagRow = -1 '-- might not need to init this @@@@@@@@@@@@@@@@@@
    ...
    If oTicket Is Nothing Then
        ...
        Set oTicket = Help_MakeTicket_Task(oMail, sClient, sTicketNum, sTopic, sSubject, sSender, sAssignee, MSGCLS_Ticket, m_fldrTickets, _
    ...
    '-- check if email is spam if it was assigned to the crap catcher
    If sClient = NOCLIENT_CLIENT And sTicketNum = NOCLIENT_TICKETNUM Then
        blnSpam = FindArrayMatch(sSubject, m_asSpamRules, iSpamRow, SPAM_PHRASES, , False)
    End If
    
    '-- move the email spam if it's spam
    If blnSpam Then
        moveMail Application.GetNamespace("MAPI").GetDefaultFolder(olFolderJunk), oMail
    
    '-- moves the mail to the mail folder
    Else
...
Private Function HELP_MakeTicket_Init() As Boolean
    ...
    If CmInitPublicGlobals Then
        ...
        m_iRowsSpamRules = CmTableLoadfromBody(m_fldrLists.Items(LIST_SPAM_RULES), m_asSpamRules)
