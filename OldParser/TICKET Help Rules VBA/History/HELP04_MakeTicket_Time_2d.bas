Attribute VB_Name = "HELP04_MakeTicket_Time_2d"
'############################################################################################################
'## HELP03_MakeTicket_Time - VBA Code to create a Time appointment item                                    2d
'############################################################################################################
...
Public Function Help_MakeTicket_Time(ByRef oAppt As AppointmentItem, _
    ...
    sInitials = CmTableFind(m_asAssignees, ASSIGNEE_NAME, sSender, ASSIGNEE_INITIALS)
    ...
    oAppt.UserProperties(TIME_BILLSTART) = CmRound15Minutes(oAppt.Start)
    oAppt.UserProperties(TIME_BILLEND) = CmRound15Minutes(oAppt.End)
    ...
    If Len(oAppt.Body) > 0 Then
        ...
        oAppt.BodyFormat = olFormatPlain
        ...
        oAppt.Body = Replace(oAppt.Body, " " & vbCrLf, vbCrLf)
    ...
    CleanUp_Time Application.GetNamespace("MAPI").GetDefaultFolder(olFolderCalendar) '-- Clean up COPY:
      
    Exit Function
ERRORHANDLER:
    CmHandleError "HELP04_MakeTicket_Time:Help_MakeTicket_Time [" & Err.Number & "] " & Err.Description & " >" & oAppt.Subject
...
'                to be removed.
'--------------------------------------------------------------------------------------------------
Private Sub CleanUp_Time(fTarget As Folder)
    ...
    Exit Sub
ERRORHANDLER:
    CmHandleError "HELP04_MakeTicket_Time:CleanUp_Time [" & Err.Number & "] " & Err.Description & " >" & fTarget.Name
    If ERR_RESUME Then Resume Next
End Sub
