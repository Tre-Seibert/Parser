Attribute VB_Name = "HELP09_DistributeMail_2s"
'############################################################################################################
...
Public Sub HELP_DistributeMail(ByRef sAssignee As String, _
    ...
    If Not blnIsItWorkTime Or blnNoHelpTechs Then
        ...
        Else
            ...
            blnDistSuccess = SendMailtoTechs(sAssignee, oMail, sDWTechs, sReason)
    ...
    If blnNoHelpTechs Or blnNoDutyTechs Or Not blnDistSuccess Then
        ...
        SendMailtoAllTechs sAssignee, oMail, sReason
    End If
    ...
    Exit Sub
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP09_DistributeMail:HELP_DistributeMail [" & Err.Number & "] " & Err.Description
...
Private Function DistributeMail_Init() As Boolean
    ...
    Exit Function
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP09_DistributeMail:DistributeMail_Init [" & Err.Number & "] " & Err.Description
    If ERR_RESUME Then Resume Next
End Function
...
Private Function FindAssignedTechs(ByRef fCalendar As Folder, _
    ...
    Exit Function
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP09_DistributeMail:FindAssignedTechs [" & Err.Number & "] " & Err.Description
    If ERR_RESUME Then Resume Next
End Function
...
Private Function FindApptbyLabel(ByRef fCalendar As Folder, _
    ...
    If cAppts.Count > 1 Then
        For idx = 1 To cAppts.Count
            If cAppts.Item(idx).RecurrenceState = olApptNotRecurring Then
...
'                        On Error GoTo 0
    ...
    Exit Function
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP09_DistributeMail:FindApptbyLabel [" & Err.Number & "] " & Err.Description
    If ERR_RESUME Then Resume Next
End Function
...
'--------------------------------------------------------------------------------------------------
Private Function SendMailtoTechs(ByRef sAssignee As String, _
    ...
    For idx = 0 To UBound(arrTechs)
        ...
        '-- Get Help Desk Techs
        sEmail = TableFind(m_asAssignees, ASSIGNEE_INITIALS, Trim(arrTechs(idx)), ASSIGNEE_EMAIL)
    ...
    If sTechEmails <> "" Then
        SendMailtoTechs = SendMail(sAssignee, oMail, sTechEmails, sReason)
    End If

    Exit Function
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP09_DistributeMail:SendMailtoTechs [" & Err.Number & "] " & Err.Description
    If ERR_RESUME Then Resume Next
End Function
...
'--------------------------------------------------------------------------------------------------
Private Function SendMailtoAllTechs(ByRef sAssignee As String, _
                                    ByRef oMail As MailItem, _
                                    ByRef sReason As String) As Boolean
    Dim sTechEmails As String
    Dim idx         As Integer
    ...
    If m_iRowsAssignees > -1 Then
        ...
        If sTechEmails <> "" Then
            SendMailtoAllTechs = SendMail(sAssignee, oMail, sTechEmails, sReason)
        End If
    End If
    
    Exit Function
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP09_DistributeMail:SendMailtoAllTechs [" & Err.Number & "] " & Err.Description
    If ERR_RESUME Then Resume Next
End Function
...
Private Function SendMailtoAssignee(ByRef sAssignee As String, _
    ...
    sTechEmails = TableFind(m_asAssignees, ASSIGNEE_INITIALS, sAssignee, ASSIGNEE_EMAIL)
    ...
    If sTechEmails <> "" Then
        ...
        If InStr(1, sTechEmails, oMail.SenderEmailAddress, vbTextCompare) = 0 Then
            ...
            '-- Send the email
            SendMailtoAssignee = SendMail(sAssignee, oMail, sTechEmails, sReason)
        End If
    End If

    Exit Function
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP09_DistributeMail:SendMailtoAssignee [" & Err.Number & "] " & Err.Description
    If ERR_RESUME Then Resume Next
End Function
...
'--------------------------------------------------------------------------------------------------
Private Function SendMail(ByRef sAssignee As String, _
                          ByRef oMail As MailItem, _
...
'    Next
    ...
    Exit Function
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP09_DistributeMail:SendMail [" & Err.Number & "] " & Err.Description
    If ERR_RESUME Then Resume Next
End Function
