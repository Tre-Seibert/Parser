Attribute VB_Name = "HELP04_MakeTicket_Time_1u"
'############################################################################################################
'## HELP03_MakeTicket_Time - VBA Code to create a Time appointment item                                    1s
'############################################################################################################
...
Public Function Help_MakeTicket_Time(ByRef oAppt As AppointmentItem, _
    ...
    sInvDesc = Format(dteWorkDate, "mm/dd/yyyy") & " " & _
               Format(oAppt.UserProperties(TIME_BILLSTART), "h:nn AM/PM") & " - " & _
               Format(oAppt.UserProperties(TIME_BILLEND), "h:nn AM/PM") & " " & _
               "(" & sInitials & ") " & _
               Trim(sTopic)
    ...
    Exit Function
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP04_MakeTicket_Time:Help_MakeTicket_Time [" & Err.Number & "] " & Err.Description
...
Sub CleanUp_Time(fTarget As Folder)
    ...
    Exit Sub
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP04_MakeTicket_Time:CleanUp_Time [" & Err.Number & "] " & Err.Description
    If ERR_RESUME Then Resume Next
End Sub
...
Function Round15Minutes(dteTime)
    ...
    Exit Function
ERRORHANDLER:
    HandleError "In Mod:Rtn HELP04_MakeTicket_Time:Round15Minutes [" & Err.Number & "] " & Err.Description
    If ERR_RESUME Then Resume Next
End Function
