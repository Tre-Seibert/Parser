Attribute VB_Name = "HELP02_MakeTicket_Task_2n"
'############################################################################################################
'## HELP01_MakeTicket_Task - VBA Code to create a Ticket task item                                         2M
'############################################################################################################
...
Public Function CreateTicket(ByRef sClient As String, _
    ...
    oTask.UserProperties.Add TKT_USED_BY, olText
    ...
    oTask.UserProperties(TKT_USED_BY) = ""
...
Public Sub Update_Ticket(ByRef oTask As TaskItem, ByRef oMail As MailItem)
    ...
    '-- Look for lock
    Set oLock = Nothing
    Set oLock = Find_Matching_Task(m_fldrLocks.Items, oTask.UserProperties(TKT_CLIENT), oTask.UserProperties(TKT_TICKETNUM))
    ...
    ElseIf Not InUse(oLock) Then
        If oLock.UserProperties(TKT_DATE_LAST_ACTIVITY) < oMail.ReceivedTime Or oLock.UserProperties(TKT_DATE_LAST_ACTIVITY) Is Nothing Then
           oTask.UserProperties(TKT_DATE_LAST_ACTIVITY) = oMail.ReceivedTime
        Else
            oTask.UserProperties(TKT_DATE_LAST_ACTIVITY) = oLock.UserProperties(TKT_DATE_LAST_ACTIVITY)
        End If
        oLock.Delete
        
    '-- update date last activity in lock and nothing else if ticket is in use
    ElseIf InUse(oLock) Then
        If oLock.UserProperties(TKT_DATE_LAST_ACTIVITY) < oMail.ReceivedTime Or oLock.UserProperties(TKT_DATE_LAST_ACTIVITY) = NODATE Then
            oLock.UserProperties(TKT_DATE_LAST_ACTIVITY) = oMail.ReceivedTime
            oLock.Save
        End If
        Exit Sub
    End If
...
Private Function InUse(ByRef oLock As TaskItem) As Boolean
    If oLock.UserProperties(TKT_USER) = "" Then
        InUse = False
    Else
        InUse = True
    End If
