Attribute VB_Name = "HELP02_MakeTicket_Task_3e"
'############################################################################################################
'## HELP01_MakeTicket_Task - VBA Code to create a Ticket task item                                         3e
'############################################################################################################
...
Public Function Help_MakeTicket_Task(ByRef oMail As MailItem, _
    ...
    If sClient = "ZZTB" Then
        oTask.Categories = TKT_CAT6_PROJECT
        oTask.UserProperties(TKT_REASON) = TKT_REASON_INTERNALPROJECT
    End If
    '-------------------------------------------------------------------------------------------
    
    '-------------------------------------------------------------------------------------------
    '-- Handle alerts/flags/etc (update fields to match data in rules lists)
    If sCategory <> "" Then oTask.Categories = sCategory
    If sStatus <> "" Then oTask.UserProperties(TKT_STATUS) = sStatus
    If sReason <> "" Then oTask.UserProperties(TKT_REASON) = sReason
    If sCause <> "" Then oTask.UserProperties(TKT_CAUSE1) = sCause
    '-------------------------------------------------------------------------------------------
    ...
    '-- Once moved, the oTask needs to point to the new object
    Set oTask = CmFindMatchingTask(fTarget.Items, sClient, sTicketNum)
    oTask.UserProperties(TKT_UNIQUEID) = oTask.EntryID
    oTask.Save
    
    '-- Return the new ticket
    Set Help_MakeTicket_Task = oTask
...
Public Function CreateTicket(ByRef sClient As String, _
    ...
    oTask.UserProperties.Add TKT_HRS_FIRST_TOUCH, olNumber
    ...
    oTask.UserProperties.Add TKT_PROJECT, olYesNo
    ...
    oTask.UserProperties(TKT_HRS_FIRST_TOUCH) = 0
    ...
    oTask.UserProperties(TKT_PROJECT) = False
